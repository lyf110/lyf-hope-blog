---
title: 三、Nacos 服务发现源码分析
date: 2023-03-28 14:54:20
order: 3
category:
  - 中间件
  - Nacos
...
![](./assets/202303081921698.png)

然后，使用IDEA将其作为一个module来导入：

1）选择项目结构选项：

![](./assets/202303081921248.png)

然后点击导入module：

![](./assets/202303081921120.png)

在弹出窗口中，选择nacos源码目录：

![](./assets/202303081921610.png)

然后选择maven模块，finish：

![](./assets/202303081921998.png)

最后，点击OK即可：

![](./assets/202303081921870.png)

导入后的项目结构：

![](./assets/202303081921255.png)

## 1.4.proto编译

Nacos底层的数据通信会基于protobuf对数据做序列化和反序列化。并将对应的proto文件定义在了consistency这个子模块中：

![](./assets/202303081921497.png)
...
Protobuf的GitHub地址：[https://github.com/protocolbuffers/protobuf/releases](https://github.com/protocolbuffers/protobuf/releases)

我们可以下载windows版本的来使用：

![](./assets/202303081921628.png)

另外，课前资料也提供了下载好的安装包：

![](./assets/202303081921079.png)

解压到任意非中文目录下，其中的bin目录中的protoc.exe可以帮助我们编译：

![](./assets/202303081922686.png)

然后将这个bin目录配置到你的环境变量path中，可以参考JDK的配置方式：

![](./assets/202303081922223.png)

### 1.4.3.编译proto

进入nacos-1.4.2的consistency模块下的src/main目录下：

![](./assets/202303081922879.png)
...
![](./assets/202303081922394.png)

会在nacos的consistency模块中编译出这些java代码：

![](./assets/202303081922075.png)

## 1.5.运行

nacos服务端的入口是在console模块中的Nacos类：

![](./assets/202303081922664.png)

我们需要让它单机启动：

![](./assets/202303081922509.png)

然后新建一个SpringBootApplication：

![](./assets/202303081922602.png)

然后填写应用信息：

![](./assets/202303081922100.png)

然后运行Nacos这个main函数：

![](./assets/202303081922881.png)

将order-service和user-service服务启动后，可以查看nacos控制台：

![](./assets/202303081922672.png)

# 2.服务注册

服务注册到Nacos以后，会保存在一个本地注册表中，其结构如下：

![](./assets/202303081923293.png)
...
![](./assets/202303081924786.png)

### 2.2.5.客户端注册的流程图

如图：

![](./assets/202303081924158.png)

## 2.3.服务端

在nacos-console的模块中，会引入nacos-naming这个模块：

![](./assets/202303081924770.png)

模块结构如下：

![](./assets/202303081924941.png)

其中的com.alibaba.nacos.naming.controllers包下就有服务注册、发现等相关的各种接口，其中的服务注册是在`InstanceController`类中：

![](./assets/202303081924725.png)
...
![](./assets/202303081925798.png)

### 3.1.2.BeatReactor

而`BeatReactor`这个类则维护了一个线程池：

![](./assets/202303081925130.png)
...
![](./assets/202303081925270.png) 
-  如果本地缓存有，则开启定时更新功能，并返回缓存结果： 
   - `scheduleUpdateIfAbsent(serviceName, clusters)`

![](./assets/202303081926698.png)在UpdateTask中，最终还是调用updateService方法：
![](./assets/202303081926864.png)
...
![](./assets/202303081926082.png)

这个是事件监听器接口，监听的是ServiceChangeEvent（服务变更事件）。

当服务列表变化时，就会通知我们：

![](./assets/202303081926249.png)
