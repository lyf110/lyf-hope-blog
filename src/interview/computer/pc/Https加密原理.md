...
本文转载于：https://zhuanlan.zhihu.com/p/43789231



![彻底搞懂HTTPS的加密原理](./assets/v2-8f6f3ee744edc28a42ca650c5fcea57f_720w.jpg)
...
![img](./assets/v2-1252526c263ee50c47b7b49bd1c2ef44_720w.webp)
...
![img](./assets/v2-1589bbfab027eb9f52da03c02a837fd4_720w.webp)
...
1. 某网站服务器拥有公钥`A-pub`与对应的私钥`A-pri`；浏览器拥有公钥`B-pub`与对应的私钥`B-pri`。
2. 浏览器把公钥`B-pub`明文传输给服务器。
3. 服务器把公钥`A-pub`明文给传输浏览器。
4. 之后浏览器向服务器传输的内容都用公钥`A-pub`加密，服务器收到后用私钥`A-pri`解密。由于只有服务器拥有私钥`A-pri`，所以能保证这条数据的安全。
5. 同理，服务器向浏览器传输的内容都用公钥`B-pub`加密，浏览器收到后用私钥`B-pri`解密。同上也可以保证这条数据的安全。

的确可以！抛开这里面仍有的漏洞不谈（下文会讲），HTTPS的加密却没使用这种方案，为什么？很重要的原因是非对称加密算法非常耗时，而对称加密快很多。那我们能不能运用非对称加密的特性解决前面提到的对称加密的漏洞？

## 7 非对称加密+对称加密？

既然非对称加密耗时，那<strong><font color='red'>`非对称加密 + 对称加密`</font></strong>结合可以吗？而且得尽量减少非对称加密的次数。当然是可以的，且非对称加密、解密各只需用一次即可。
请看一下这个过程：

1. 某网站拥有用于非对称加密的公钥`A-pub`、私钥`A-pri`。
2. 浏览器向网站服务器请求，服务器把公钥`A-pub`明文给传输浏览器。
3. 浏览器随机生成一个用于对称加密的密钥`X`，用公钥`A-pub`加密后传给服务器。
4. 服务器拿到后用私钥`A-pri`解密得到密钥X。
...
![img](./assets/v2-daeb6b8b0ac5411d51d60bd08b551789_720w.webp)

中间人攻击（https://blog.pradeo.com/man-in-the-middle-attack）

如果在数据传输过程中，中间人劫持到了数据，此时他的确无法得到浏览器生成的密钥X，这个密钥本身被公钥`A-pub`加密了，只有服务器才有私钥`A-pri`解开它，然而中间人却完全不需要拿到私钥`A-pri`就能干坏事了。请看：

1. 某网站有用于非对称加密的公钥`A-pub`、私钥`A-pri`。
2. 浏览器向网站服务器请求，服务器把公钥`A-pub`明文给传输浏览器。
3. **中间人劫持到公钥`A-pub`，保存下来，把数据包中的公钥`A-pub`替换成自己伪造的公钥`B-pub`（它当然也拥有公钥`B-pub`对应的私钥`B-pri`）**。
4. 浏览器生成一个用于对称加密的密钥X，用**公钥`B-pub`**（浏览器无法得知公钥被替换了）加密后传给服务器。
5. **中间人劫持后用私钥`B-pri`解密得到密钥X，再用公钥`A-pub`加密后传给服务器**。
6. 服务器拿到后用私钥`A-pri`解密得到密钥X。
...
![img](./assets/v2-1123c51b7d4a0db707100971b91728a9_720w.webp)
...
![img](./assets/v2-7c78935389af46e197e96d9cd91c06dd_720w.webp)
...
![img](./assets/v2-04cd27f3f46388df2d8d70375c4ecac5_720w.webp)

信任链（https://publib.boulder.ibm.com/tividd/td/TRM/GC32-1323-00/en_US/HTML/admin230.htm）

## 12 每次进行HTTPS请求时都必须**在SSL/TLS层进行握手传输密钥吗？**
...
![img](./assets/v2-a0d10af45e785fe8d3f5cd12f8c309f5_720w.webp)
